...
/**
 * Minecraft 26.2 rejects serverbound custom payloads larger than 32767 bytes.
 * Axiom 5.5.0 (API 9) still sends large plugin messages on individual channels,
 * so those packets are intercepted here before the vanilla decoder.
 */
public class AxiomBigPayloadHandler extends MessageToMessageDecoder<ByteBuf> {
            ...
            if (packetId == this.payloadId) {
                ...
                if (handler != null) {
                    ...
                    if (handler.handleAsync()) {
                        ...
                        ServerScheduler.executeNowOrForEntity(AxiomPaper.PLUGIN, bukkitPlayer, () -> {
    ...
    private static void callReceive(PacketHandler handler, ServerPlayer player, RegistryFriendlyByteBuf friendlyByteBuf, String identifier) {
        ...
        try {
            ...
            player.connection.disconnectAsync(
                    net.minecraft.network.chat.Component.literal("Error while processing Axiom packet " + identifier + ": " + t.getMessage()),
